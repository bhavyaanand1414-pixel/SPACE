"""
Landslide Scar and Bare-Earth Slope Change Assessment Module.
"""
...
class LandslideScarAnalyzer(BaseDisasterAnalyzer):
    """
    Assesses potential landslide-related slope exposure, runout paths, and terrain scar.
    """

    def __init__(self, model_version: str = "1.0.0"):
        super().__init__(disaster_type=DisasterType.LANDSLIDE, model_version=model_version)
    ...
    def analyze(
        ...
        **kwargs,
        ...
        """Analyze bare-earth soil exposure on sloped terrain."""
        ...
        # Vegetation loss + soil reflectance surge
        if c >= 4:
            ...
            scar_mask = ((delta_ndvi < -0.25) & (image_after[0] > image_before[0])).astype(np.uint8) * 255
        else:
            diff = image_after[0] - image_before[0]
            scar_mask = (diff > 0.25).astype(np.uint8) * 255

        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        clean_scar = cv2.morphologyEx(scar_mask, cv2.MORPH_OPEN, kernel)

        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(clean_scar, connectivity=8)
            ...
            for cnt in contours:
                ...
                feat = {
                    ...
                    "properties": {
                        ...
                        "impact_type": "Potential Landslide Scar",
                        "area_m2": round(reg_area_m2, 2),
                        "area_km2": round(reg_area_km2, 4),
                        "confidence": 0.86,
                        "causality_wording": "Potential landslide-related change",
                    },
                }
                geojson_features.append(feat)
                region_idx += 1

        scar_px = int(np.count_nonzero(clean_scar))
        affected_m2 = float(scar_px * pixel_area_m2)
